...
using static DoctorsList.Abstractions.IBaseRepository;
...
namespace DoctorsList.Repositories
{
      ...
      IBaseRepository<T> where T : TableData, new()
    {
        ...
        public T GetItem(int id, bool recursive = false)
        {
            try
            {
                return
                     connection.GetWithChildren<T>(id, recursive);
            }
            catch (Exception ex)
            {
                StatusMessage =
        ...
        public List<T> GetItemsWithChildren()
        {
            try
            {
                var result = connection.GetAllWithChildren<T>(null,true);
                return result;
            }
            catch (Exception ex)
            {
